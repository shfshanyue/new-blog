---
title: "关于 async/await 的两个 OOM 的示例：内存溢出问题分析与解决"
description: "分析两个在开发过程中因为使用 async/await 而导致的内存溢出(OOM)问题，包括 Promise.map 并发控制失效和递归调用导致的内存泄漏案例"
pubDatetime: 2019-05-20T16:00:00+08:00
tags:
  [
    "javascript",
    "async",
...
async function populatePoemsPage(page) {
  ...
  await Promise.map(
    ...
    async poem => {
      // 原因: 在于这里忘记了点 await
      Promise.map(
        poem.raw_paragraphs.match(/[\u4e00-\u9fa5]/g) || [],
        async char => {
...
async function scan(cursor) {
  const [nextCursor, list] = await redis.sscan(
    "CharCloudList",
    cursor,
    "MATCH",
    "*",
