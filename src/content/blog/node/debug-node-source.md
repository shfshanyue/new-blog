...
description: "node.js 源码托管在 $1。 通过以下命令对 node.js 源码进行编译。 **等待的时间较长，需要一顿吃火锅的功夫。** 源码编译之后会生成 ./out 目录，在根目录中会生成可执行命令行工具 node 与 node_g ，指向 ./out 中的符号链接。 **使用 node_g 替代全局的 node ，可对源码进行调试。**"
