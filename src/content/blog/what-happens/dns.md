---
title: "DNS (Domain Name System) 解析"
description: "DNS 用来解析域名所对应的 IP 地址。 > 我们要以命令洞见整个流程 DNS 协议属于应用层协议，使用 dig 命令可以发送一条 DNS 报文。既然它在进行域名解析时实质是发送了一条 DNS 的报文，那首先来简单看看报文格式 > dig: DNS lookup utility > https://www.thegeekstuff.co"
